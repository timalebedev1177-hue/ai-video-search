...
print(
    f"Embeddings сохранены: {embeddings_path}"
)

print(
    f"Размер embeddings: {all_embeddings.shape}"
)

# Проверка соответствия
if len(all_embeddings) != len(all_meta):
    raise RuntimeError(
        f"Ошибка: embeddings ({len(all_embeddings)}) "
        f"не совпадают с metadata ({len(all_meta)})"
    )
